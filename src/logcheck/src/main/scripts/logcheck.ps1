...
if( $serviceInstall )
{
	$installStr = "$procRunExe //IS/LOGCHECK  --Classpath=""$javaJarPath"" --DisplayName=""Logcheck"" --StartMode=jvm --StartClass=$javaStartClass --StartMethod=windowsStart --StopMethod=windowsStop --JavaHome=""$javaHome"" --Jvm=""$javaJVM"" --StdOutput=auto --StdError=auto --StartParams=--service $javaArgsArray $serviceArgs"
